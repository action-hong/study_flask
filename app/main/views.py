...
from ..decorators import admin_required, permission_require
...
@main.route('/moderate')
@login_required
@permission_require(Permission.MODERATE_COMMENTS)
def moderate():
    page = request.args.get('page', 1, type=int)
...
def post(id):
    ...
    if form.validate_on_submit():
        comment = Comment(post=post, author=current_user._get_current_object(), body=form.body.data)
        db.session.add(comment)
...
def user(username):
    ...
    return render_template('user.html', user=user,
                           endpoint='.user', posts=posts, pagination=pagination)
...
def unfollow(username):
    ...
    if not current_user.is_following(user):
        flash('you are already unfollow the user')
    else:
